...
if ($customerid && $autoid && $paydate && $transacid && $duedate && $paid && $f==0) 
{
	...
	mysql_connect("localhost:3306","root", "mysql") or die("Problem with connection...");
	mysql_select_db("data") or die(mysql_error());
	$pname=mysql_fetch_assoc(mysql_query("SELECT POLICY_NAME FROM policy WHERE POLICY_ID='$autoid'"))['POLICY_NAME'];
	...
	mysql_query("INSERT INTO premium_tr_policy(premium_tr_id,policy_id,policy_name,customer_id) VALUES($transacid,'$autoid','$pname','$customerid')");
