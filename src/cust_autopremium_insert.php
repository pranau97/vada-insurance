...
if ($customerid && $autoid && $paydate && $transacid && $duedate && $paid && $f==0) 
{

	$autoid = $_POST['autoid'];
	$url=parse_url(getenv("CLEARDB_DATABASE_URL"));    $server = $url["host"];   $username = $url["user"];   $password1 = $url["pass"];   $db = substr($url["path"],1);   $con= mysqli_connect($server, $username, $password1) or die("Problem with connection...");
	mysqli_select_db($con,$db) or die(mysqli_error($con));
	$pname=mysqli_fetch_assoc(mysqli_query($con, "SELECT POLICY_NAME FROM policy WHERE POLICY_ID='$autoid'"))['POLICY_NAME'];
	...
	mysqli_query($con, "INSERT INTO premium_tr_policy(premium_tr_id,policy_id,policy_name,customer_id) VALUES($transacid,'$autoid','$pname','$customerid')");
